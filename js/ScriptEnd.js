...
function LoadMapData(datas){ 
	...
	highest_value=0;
	for(i=0; i < regions.length; i++) {
		if(Number.isNaN(parseInt(regions[i].inNeeds))==false){
    ...
    for(i=0; i < regions.length; i++) {
        ...
        if(highest_value==0){
			...
			if(Number.isInteger(parseInt(valInneed))==false){
				valInneed = 0;
			}
...
$('#carte g polygon').mouseover(function (e) {
    ...
    if(!Number.isNaN(parseInt(inNedd))){
	...
	if(!Number.isNaN(parseInt(requier))){
    ...
    $('#carte #Regions polygon').each(function() {
        ...
        if(Number.isNaN(parseInt(regionData.inNeeds))==false){
